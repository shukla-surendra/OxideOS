...
## Full Attribute Byte Chart

Each attribute byte is `0xXY`, where `X` = background, `Y` = foreground:
...
Examples: `0x1F` → Blue background, White foreground · `0x4E` → Red background, Yellow foreground · `0x70` → Light Gray background, Black foreground
