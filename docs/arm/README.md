...
| Disk | ATA PIO (ports `0x1F0`…) | virtio-blk (virtio-mmio, polled) |
...
# persistent storage: create the FAT16 disk image once — every aarch64 run
# target then attaches it as a virtio-blk device automatically
make disk
```
